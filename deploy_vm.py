...
def startup(config: dict) -> str:
    ...
    return STARTUP.replace("@@REALITY@@", values["reality"]).replace("@@OVPN@@", values["ovpn"])
        .replace("@@SSH@@", values["ssh"]).replace("@@RPORT@@", values["reality_port"])
        .replace("@@DEST@@", values["reality_dest"]).replace("@@SNI@@", values["reality_sni"])
        .replace("@@OPORT@@", values["ovpn_port"]).replace("@@OPROTO@@", values["ovpn_proto"])
...
def cloudflare(domain: str, subdomain: str, ip: str, token: str) -> str | None:
    # This intentionally uses the last two labels. For multi-level public
    # suffixes, create the record manually or extend this function safely.
...
if [ "$REALITY" = true ]; then
  ...
  UUID=$(cat /proc/sys/kernel/random/uuid); KEYS=$(/usr/local/bin/xray x25519)
  PRIVATE=$(awk -F': ' '/Private key/{print $2}' <<<"$KEYS"); PUBLIC=$(awk -F': ' '/Public key/{print $2}' <<<"$KEYS"); SID=$(openssl rand -hex 8)
  mkdir -p /usr/local/etc/xray
  jq -n --arg uuid "$UUID" --arg private "$PRIVATE" --arg dest "$DEST" --arg sni "$SNI" --arg sid "$SID" --argjson port "$RPORT" '{log:{loglevel:"warning"},inbounds:[{listen:"0.0.0.0",port:$port,protocol:"vless",settings:{clients:[{id:$uuid,flow:"xtls-rprx-vision"}],decryption:"none"},streamSettings:{network:"tcp",security:"reality",realitySettings:{show:false,dest:$dest,xver:0,serverNames:[$sni],privateKey:$private,shortIds:[$sid]}}}],outbounds:[{protocol:"freedom"}]}' > /usr/local/etc/xray/config.json
  systemctl enable --now xray
  PUBLIC=${PUBLIC:-$(awk -F': ' '/Public key/{print $2}' <<<"$KEYS")}
  if systemctl is-active --quiet xray; then REALITY_JSON=$(jq -n --arg uuid "$UUID" --arg pbk "$PUBLIC" --arg sid "$SID" --arg sni "$SNI" --arg dest "$DEST" --argjson port "$RPORT" '{enabled:true,uuid:$uuid,publicKey:$pbk,shortId:$sid,serverName:$sni,dest:$dest,port:$port}'); fi
fi
...
if [ "$OVPN" = true ]; then
  ...
  ./easyrsa init-pki; ./easyrsa --batch build-ca nopass; ./easyrsa --batch gen-req server nopass; ./easyrsa --batch sign-req server server; ./easyrsa --batch gen-req client1 nopass; ./easyrsa --batch sign-req client client1; ./easyrsa gen-dh; openvpn --genkey tls-crypt "$EZ/tc.key"
...
EOF
  ...
  echo 'net.ipv4.ip_forward=1' > /etc/sysctl.d/99-openvpn.conf; sysctl --system >/dev/null
  ...
  netfilter-persistent save >/dev/null 2>&1 || true; systemctl enable --now openvpn-server@server
  pem(){ sed -n '/-----BEGIN/,/-----END/p' "$1"; }
  { echo "client"; echo "dev tun"; echo "proto $OPROTO"; echo "remote $IP $OPORT"; echo "nobind"; echo "persist-key"; echo "persist-tun"; echo "remote-cert-tls server"; echo "cipher AES-256-GCM"; echo "auth SHA256"; echo '<ca>'; pem "$EZ/pki/ca.crt"; echo '</ca>'; echo '<cert>'; pem "$EZ/pki/issued/client1.crt"; echo '</cert>'; echo '<key>'; pem "$EZ/pki/private/client1.key"; echo '</key>'; echo '<tls-crypt>'; cat "$EZ/tc.key"; echo '</tls-crypt>'; } > /root/client1.ovpn; chmod 600 /root/client1.ovpn
  systemctl is-active --quiet openvpn-server@server && OVPN_JSON=$(jq -n --arg proto "$OPROTO" --argjson port "$OPORT" '{enabled:true,proto:$proto,port:$port}')
fi

if [ "$SSH_TUNNEL" = true ]; then
  id tunnel >/dev/null 2>&1 || useradd -m -s /usr/sbin/nologin tunnel; mkdir -p /home/tunnel/.ssh; printf '%s\n' "$PUBKEY" > /home/tunnel/.ssh/authorized_keys; chmod 700 /home/tunnel/.ssh; chmod 600 /home/tunnel/.ssh/authorized_keys; chown -R tunnel:tunnel /home/tunnel/.ssh
...
EOF
  systemctl restart ssh; SSH_JSON='{"enabled":true,"user":"tunnel","port":22}'
fi
jq -n --argjson reality "$REALITY_JSON" --argjson openvpn "$OVPN_JSON" --argjson ssh "$SSH_JSON" --arg ip "$IP" '{status:"ready",external_ip:$ip,reality:$reality,openvpn:$openvpn,ssh_tunnel:$ssh}' > /etc/vm-setup-complete.json
'''
...
def main() -> None:
    ...
    project_id = project(); zone = ask("GCE zone", "us-central1-a"); region = "-".join(zone.split("-")[:-1])
    name = ask("VM name", "saeka-tcp"); machine = ask("Machine type", "e2-micro"); tag = f"{name}-tcp"
    ...
    if not (reality or ovpn or ssh_tunnel): die("select at least one protocol")
    ...
    if oproto not in {"udp", "tcp"}: die("OpenVPN protocol must be udp or tcp")
    key_path = WORKDIR / f"{name}_tunnel_key"; WORKDIR.mkdir(parents=True, exist_ok=True)
    if ssh_tunnel and not key_path.exists(): cmd(["ssh-keygen", "-t", "ed25519", "-N", "", "-f", str(key_path), "-q"])
    pubkey = (key_path.with_suffix(".pub").read_text().strip() if ssh_tunnel else "")
    ip = regional_ip(project_id, region, f"{name}-ip")
    if reality: firewall(project_id, f"{tag}-reality", "tcp", rport, tag)
    if ovpn: firewall(project_id, f"{tag}-openvpn", oproto, oport, tag)
    firewall(project_id, f"{tag}-iap", "tcp", 22, tag, "35.235.240.0/20")
    cfg = {"reality": str(reality).lower(), "ovpn": str(ovpn).lower(), "ssh": str(ssh_tunnel).lower(), "reality_port": rport, "reality_dest": dest, "reality_sni": sni, "ovpn_port": oport, "ovpn_proto": oproto, "pubkey": pubkey}
    script_path = WORKDIR / f"{name}-startup.sh"; script_path.write_text(startup(cfg)); script_path.chmod(0o700)
    create_vm(project_id, zone, name, machine, ip, tag, script_path); result = wait_ready(project_id, zone, name)
    domain = ask("DNS domain for VM sibling record (blank to skip)", ""); fqdn = None
    if domain:
        subdomain = ask("VM subdomain", "vpn"); token = os.environ.get("CF_API_TOKEN") or getpass.getpass("Cloudflare DNS API token (blank to skip): ")
        if token: fqdn = cloudflare(domain, subdomain, ip, token)
    host = fqdn or ip; print(colour(f"\nVM ready: {host} (static IP {ip})", GREEN))
    if result.get("reality", {}).get("enabled"):
        r = result["reality"]; print(f"REALITY: vless://{r['uuid']}@{host}:{r['port']}?encryption=none&flow=xtls-rprx-vision&security=reality&sni={r['serverName']}&fp=chrome&pbk={r['publicKey']}&sid={r['shortId']}&type=tcp#saeka-reality")
    if ovpn and scp(project_id, zone, name, "/root/client1.ovpn", WORKDIR / f"{name}-client1.ovpn"):
        (WORKDIR / f"{name}-client1.ovpn").chmod(0o600); print(f"OpenVPN profile: {WORKDIR / f'{name}-client1.ovpn'}")
    if ssh_tunnel: print(f"SSH SOCKS: ssh -i {key_path} -D 1080 -N tunnel@{host}")
    print(colour("Cloud Run remains the HTTP endpoint; use the VM host for TCP/UDP protocols.", YELLOW))


if __name__ == "__main__":
    try: main()
    except KeyboardInterrupt: print("\nAborted."); raise SystemExit(130)
