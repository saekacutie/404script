...
echo "[+] Starting caddy..."
caddy run --config /etc/caddy/Caddyfile --adapter caddyfile & ENGINE_PID=$!
...
while true; do
  ...
  if ! kill -0 "$ENGINE_PID" 2>/dev/null; then caddy run --config /etc/caddy/Caddyfile --adapter caddyfile & ENGINE_PID=$!; fi
done
