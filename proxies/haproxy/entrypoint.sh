...
haproxy -f /usr/local/etc/haproxy/haproxy.cfg -db & ENGINE_PID=$!
...
while true; do
  ...
  if ! kill -0 "$ENGINE_PID" 2>/dev/null; then haproxy -f /usr/local/etc/haproxy/haproxy.cfg -db & ENGINE_PID=$!; fi
done
